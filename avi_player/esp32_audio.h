...
extern String previous_avi_filename;
...
void audioDataCallback(MP3FrameInfo &info, int16_t *pwm_buffer, size_t len, void *ref) {
  ...
  float currentGain = isMuted ? 0.0 : GAIN_LEVEL * 0.05;
  for (int i = 0; i < len; i++)
  {
      pwm_buffer[i] = pwm_buffer[i] * currentGain;
  }
...
void mp3_player_task(void *pvParam) {
  ...
  } while (audbuf_read > 0 && isPlaying && previous_avi_filename == avi_filename);
...
void mp3_player_task_remove() {
  if (mp3PlayerTaskHandle != NULL) {
    vTaskDelete(mp3PlayerTaskHandle);  // 删除任务
    mp3PlayerTaskHandle = NULL;        // 将句柄置为 NULL，表示任务已删除
    Serial.println("MP3 player task deleted.");
  }
}
...
BaseType_t mp3_player_task_start() {
  ...
  return xTaskCreatePinnedToCore(
    ...
    (TaskHandle_t *const)&mp3PlayerTaskHandle,
    (const BaseType_t)0);
}
